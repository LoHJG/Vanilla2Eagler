...
class LanTcpBridge {
  ...
  start() {
    ...
    this.socket = net.connect({ host: "127.0.0.1", port }, () => {
      this.readLoop().catch((err) => {
        if (!this.closed) this.peer.owner.server.emit("log", { level: "warn", message: `World bridge error: ${err.message}` });
        this.close();
  ...
  detectProtocolVersion(raw) {
    try {
      let off = 0;
      const packetId = readVarInt(raw, off);
  ...
  onClientFrame(frame) {
    ...
    if (raw) {
      if (!this.handshakeSent) {
        ...
        this.peer.owner.server.emit("log", { level: "info", message: `World peer detected Minecraft protocol ${this.detectedProtocolVersion}` });
...
class WorldPeer {
  ...
  create(iceServers) {
    ...
    pc.onDataChannel.subscribe((dc) => {
      this.owner.server.emit("log", { level: "info", message: `World peer ${this.id} data channel arrived` });
...
export class EaglerRelayServer extends EventEmitter {
  constructor(options = {}) {
    ...
    this.name = options.name || "§aVanilla2Eagler Network §c[1.8.9/1.12.2/1.20.6]";
